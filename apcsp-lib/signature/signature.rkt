...
(struct signature 
  (name enforcer syntax-promise (arbitrary-promise #:mutable) info-promise <=?-proc =?-proc)
  #:constructor-name really-make-signature
  #:property prop:equal+hash
  (list (λ (c1 c2 _) (signature=? c1 c2)) ; #### use equal?
	(λ (r recur)
...
(define (make-signature name enforcer syntax-promise
                        ...
                        #:<=?-proc (<=?-proc (lambda (this-info other-info) #f))
                        #:=?-proc (=?-proc (lambda (this-info other-info) #f)))
  (really-make-signature name enforcer 
                         syntax-promise arbitrary-promise info-promise 
                         <=?-proc =?-proc))
...
(struct exn:fail:contract:signature exn:fail:contract 
  (obj signature blame-srcloc) 
  #:transparent
  #:extra-constructor-name make-exn:fail:contract:signature)

; message may be #f
(define signature-violation-proc
  (make-parameter
   (λ (obj signature message blame-srcloc)
...
(define (check-signature sig val success fail)
  ((let/ec exit
     (let ([enforced (call-with-signature-violation-proc
                      (λ (signature syntax msg blame)
                        (exit fail))
                      (λ ()
...
(define (signature->predicate sig)
  (λ (val)
    (check-signature sig val (λ (_) #t) (λ () #f))))

(struct procedure-to-blame 
  (proc srcloc)
  #:extra-constructor-name make-procedure-to-blame
...
(struct type-variable-info ()
  #:extra-constructor-name make-type-variable-info)
