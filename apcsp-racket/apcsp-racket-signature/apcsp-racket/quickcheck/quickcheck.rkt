...
(require racket/promise "random.rkt") ; had srfi/9 before
...
(struct generator (proc) #:constructor-name make-generator)
...
(struct arbitrary (generator transformer) #:constructor-name make-arbitrary)
...
(struct :property (proc arg-names args) #:constructor-name make-property)
...
(struct config (max-test max-fail size print-every) #:constructor-name make-config)
